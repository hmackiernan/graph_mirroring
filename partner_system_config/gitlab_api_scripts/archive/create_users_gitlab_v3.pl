...
my $url = $server_hr->{"server_name"}{"url"};
my $user = $server_hr->{"server_name"}{"admin_user"};
my $pass = $server_hr->{"server_name"}{"admin_pass"};
...
my $desired_users_hr = { "alice" => {"level" => $GITLAB_ACCESS_LEVEL_MASTER,
			 ...
			 "service_user" => {"level" => $GITLAB_ACCESS_LEVEL_DEVELOPER,
					    "id" => undef},
			 "root" => {"level" => $GITLAB_ACCESS_LEVEL_DEVELOPER,
					    "id" => undef},
