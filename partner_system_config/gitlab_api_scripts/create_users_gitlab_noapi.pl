# Updated to not use the GitLab perl API
...
my ($GITLAB_ACCESS_LEVEL_DEVELOPER,$GITLAB_ACCESS_LEVEL_MASTER) = qw(40 30);

my %opt = ("server_name" => "gitlabee9",
	  );

GetOptions(\%opt,"server_name:s","debug:s");
...
# and add keys etc.
my $desired_users_hr = { "dan" => {"level" => $GITLAB_ACCESS_LEVEL_MASTER,
				     "password" =>"password",
				     "id"=> undef},
			 "loretta" =>{"level" => $GITLAB_ACCESS_LEVEL_MASTER,
				     "password" =>"password",
				     "id"=> undef},
			 

			 
			 "service_users" => {"level" => $GITLAB_ACCESS_LEVEL_DEVELOPER,
					    "password" =>"password",
					    "id" => undef},

			 "disservice_user" => {"level" => $GITLAB_ACCESS_LEVEL_DEVELOPER,
					       "password" =>"password",
					       "id" => undef},
...
foreach my $user (sort(keys(%{$desired_users_hr}))) {
  ...
  my $ret = create_user($url,$token,$user,$desired_users_hr->{$user}{"password"},$email);
  print Dumper($ret);
}
...
#  my $api = shift;
  ...
  my $query = $url . "/users?username=$user";
  print "Query is $query";
  print "\n";
  my $response = $ua->get($query,"PRIVATE-TOKEN" => $token);
  my $resp_obj = parse_json ($response->decoded_content);
  $ua = undef;
  $response = undef;
  $query = undef;

  if (scalar(@{$resp_obj}) == 1) {
    #   # if exists, return with ID of already existing user
    $msg =  "user $user exists with id $resp_obj->[0]{id}";
    $id = $resp_obj->[0]{id};
  } else {
    $msg = "$user does not exist, creating";
    $ua = LWP::UserAgent->new();
    $query = $url . "/users?private_token=$token";
         my $ua = LWP::UserAgent->new();
     my $form = {
  		  "email" =>  $email,
  		  "username" => $user,
    ...
    $response = $ua->post($query,$form);
    if ($response->is_success) { 
      my $resp_obj = parse_json ($response->decoded_content);
      print "This is what create does\n";
      $msg = "Created with $resp_obj->{'id'}";
      $id = $resp_obj->{'id'};
    } else {
